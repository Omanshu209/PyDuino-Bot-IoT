...
MOTOR_STOP = 3
TCP_IP = '192.168.43.196'  # replace this with the IP address of ESP32
TCP_PORT = 5005  # replace this with the port number used on the ESP32
BUFFER_SIZE = 1024
...
# A function to send data to ESP32 over Wi-Fi
def sendDataToESP(data):
	try:
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.MOUSEBUTTONDOWN:
			...
			if MX >= 90 and MX <= 260 and MY >= 570 and MY <= 700:
				sendDataToESP(MOTOR_ANTICLOCKWISE)
				CAR1.RIGHT = False
				CAR1.LEFT = True
			elif MX >= 960 and MX <= 1130 and MY >= 570 and MY <= 700:
				sendDataToESP(MOTOR_CLOCKWISE)
				CAR1.LEFT = False
				CAR1.RIGHT = True
